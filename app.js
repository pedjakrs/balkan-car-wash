...
/* =========================================================================
   ...
   ========================================================================= */
const SHEETS = ['Vozila','Klijenti','Usluge','Normativi','Magacin','MagacinTx','Nalozi','Radnici','Troskovi','Prihodi','Reklamacije','Subote','Bonusi','Podesavanja'];
...
function buildNav(){
  ...
  NAV.forEach(item=>{
    if(item.group){
      ...
      if(item.adminOnly) g.dataset.adminOnly='1';
      nav.appendChild(g); return;
    }
    const b=document.createElement('button');
    b.dataset.nav=item.id; if(item.adminOnly) b.dataset.adminOnly='1';
...
function bindChrome(){
  ...
  $$('.role-toggle button').forEach(b=> b.onclick=()=>{
    State.role=b.dataset.role; document.body.dataset.role=State.role;
    $$('.role-toggle button').forEach(x=>x.classList.toggle('is-active',x===b));
    if(State.role==='radnik' && NAV.find(n=>n.id===State.view)?.adminOnly) route('kalendar');
    else route(State.view);
  });
...
function renderEntity(el,name){
  ...
  const toolbar=`
    <div class="toolbar">
      ...
      <button class="btn primary" id="entAdd" data-admin-only>${svgIco('grid').replace('grid','')}<svg viewBox="0 0 24 24" width="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M12 5v14M5 12h14"/></svg>${cfg.add}</button>
    </div>`;
